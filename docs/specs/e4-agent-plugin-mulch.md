...
4. Push non-empty `ml prime` output directly to `context.promptSegments`
...
Raw markdown generated by `ml prime` is injected directly into `context.promptSegments`.
...
- [x] `mulchPlugin.onSessionStart` injects `ml prime` output into `context.promptSegments`
