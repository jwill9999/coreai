...
import type { RuntimeContext } from './public-types.js';
...
function makeContext(taskId?: string): RuntimeContext {
  return {
    repoRoot: REPO,
    config: {},
    memorySegments: [],
...
describe('HOOK_SCRIPT_NAMES', () => {
  it('maps v3 lifecycle methods', () => {
    expect(HOOK_SCRIPT_NAMES.onSessionStart).toBe('session-start');
    expect(HOOK_SCRIPT_NAMES.onTaskStart).toBe('task-start');
    expect(HOOK_SCRIPT_NAMES.onMemoryCompose).toBe('memory-compose');
    expect(HOOK_SCRIPT_NAMES.onSessionEnd).toBe('session-end');
  });
});
...
describe('HookRunner.isApprovedWrite()', () => {
  ...
  it('treats allowWrite entries ending in / as approved directory prefixes', () => {
    const runner = new HookRunner(REPO, {
      permissions: { allowWrite: ['.mulch/expertise/'] },
    });
    expect(runner.isApprovedWrite('.mulch/expertise/testing.jsonl')).toBe(true);
  });

  it('does not match sibling paths for approved directory prefixes', () => {
    const runner = new HookRunner(REPO, {
      permissions: { allowWrite: ['.mulch/expertise/'] },
    });
    expect(runner.isApprovedWrite('.mulch/expertise-notes/testing.jsonl')).toBe(
      false,
    );
  });
