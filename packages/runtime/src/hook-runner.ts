...
/** Maps plugin hook names to repo/global hook script stem names. */
export const HOOK_SCRIPT_NAMES = {
  onSessionStart: 'session-start',
  onTaskStart: 'task-start',
  onMemoryCompose: 'memory-compose',
  onSessionEnd: 'session-end',
} as const;

export type HookScriptKey = keyof typeof HOOK_SCRIPT_NAMES;
...
export class HookRunner {
   ...
   */
  async runHook(hookName: string, context: RuntimeContext): Promise<void> {
  ...
  private static promptWritePermissions(
    ...
    return new Promise((res) => {
      ...
      rl.question(
        `@conscius/cli: Allow hooks to write to approved paths (${list})? [y/N] `,
        (answer) => {
          rl.close();
